...
Lista* insere(Lista* l, int id, char new_nome[], int new_idade, char new_cpf[], float new_pontos){
	...
	strcpy(dados->nome, new_nome);
	dados->idade = new_idade;
	strcpy(dados->cpf, new_cpf);
	dados->pontos = new_pontos;
...
Lista* retira (Lista* l, char nome[]) {
	...
	while (p != NULL && strcmp(nome, p->infos->nome)) {
		ant = p;
		p = p->prox;
	}
	if (p == NULL)
...
void imprime (Lista* l) {
	...
	for(p = l; p != NULL; p = p->prox){
		if(p != NULL){
			if(!vazia(l)){
				strcpy(dados[i-1].nome, p->infos->nome);
				dados[i-1].idade = p->infos->idade;
				strcpy(dados[i-1].cpf, p->infos->cpf);
				dados[i-1].pontos = p->infos->pontos;
				i--;
			}
		}
	}
